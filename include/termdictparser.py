	...
	def build_vocabulary(self, tokens=[], re_tokenizer=r"[a-zA-Z0-9(-+.)']+"):
		...
		for id, token in tokens:
			...
			for i, word in enumerate(wordlist, start=1):
				...
				# NodeIdentifier
				if i == 1:
					identifier = u'-'.join([unicode(i)+'.'+word])
				else:
					identifier = u'-'.join( [nodes[-1].identifier] + [unicode(i)+'.'+word] )
...
if __name__ == '__main__':
	...
	terms = [
			(01, u'asthma/bronchitis'),
			(02, u'linhaça'), # unicode characteç'
			('D03', u'ADHD'), # acym
			('D02', u'N.E.E.'), # acym
			('DB01', u'fluoxetine'),
			(03, u'weight loss diet'),
			(04, u'weight gain diet'),
			(03, u'losing weight'),
			(03, u'weight loss'),
			(10, u'brain cancer'),
